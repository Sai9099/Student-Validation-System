...
public class StudentRegisterForm : Form
{
    ...
    private readonly TextBox _registerNumber = new();
    ...
    {
        Text = "Student Register - Student Learning Evidence & Progress Validation System";
    ...
    private void Build()
    {
        ...
        var title = Ui.Label("Student Registration", Ui.TitleFont, _theme.Text);
        title.Location = new Point(32, 24);
        card.Controls.Add(title);

        var y = 78;
        AddField(card, "Register Number", _registerNumber, 32, y);
        AddField(card, "Full Name", _fullName, 292, y);
        y += 76;
        AddField(card, "Department", _department, 32, y);
        AddCombo(card, "Semester", _semester, 292, y);
        y += 76;
        AddField(card, "Section", _section, 32, y);
        AddField(card, "Email", _email, 292, y);
        y += 76;
        AddField(card, "Mobile Number", _mobile, 32, y);
        AddField(card, "Password", _password, 292, y, true);
        y += 76;
        AddField(card, "Confirm Password", _confirmPassword, 32, y, true);

        _showPassword.Text = "Show password";
        _showPassword.Location = new Point(292, y + 28);
        ...
        register.Location = new Point(32, 430);
    ...
    {
        _message.Text = "";
        var result = _auth.RegisterStudent(
            _registerNumber.Text,
            _fullName.Text,
            _department.Text,
            _semester.Text,
            _section.Text,
        ...
        MessageBox.Show("Registration successful", "Student Registration", MessageBoxButtons.OK, MessageBoxIcon.Information);
        Hide();
        var login = new StudentLoginForm();
    ...
    private void AddCombo(Control parent, string label, ComboBox combo, int x, int y)
    {
        AddLabel(parent, label, x, y);
        combo.Location = new Point(x, y + 26);
        combo.Width = 230;
